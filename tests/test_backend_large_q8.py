...
def test_backend_large_q8(tmp_path):
    # Create a larger weight matrix to exercise AVX2 path for q8
    rows = 33
    cols = 67
    np.random.seed(43)
    ...
    q_path = manifest_dir / 'large_q8.npy'
    scales_path = manifest_dir / 'large_q8_scales.npy'
    zps_path = manifest_dir / 'large_q8_zero_point.npy'
    np.save(q_path, q)
    ...
    cmd = ['python', 'cpp/call_backend.py', '--manifest', str(manifest_path), '--weight', 'large_w_q8', '--input', str(input_path), '--out-json']
