...
def test_call_backend_q8(tmp_path):
    # Create a small weight matrix and quantize to q8
    W = np.array([[0.3, 0.6, -0.7, 1.4],
                  [-0.5, 0.2, 0.0, 0.9]], dtype=np.float32)
    q, scales, zps = quantize_per_row(W, bits=8)
    manifest_dir = Path(tmp_path)
    q_path = manifest_dir / 'myw_q8.npy'
    scales_path = manifest_dir / 'myw_q8_scales.npy'
    zps_path = manifest_dir / 'myw_q8_zero_point.npy'
    np.save(q_path, q)
