    ...
    if key_1bit in npz:
        ...
        if key_pc in npz:
            ...
            if scales.shape[0] == shape[0]:
                ...
                raise ValueError(f"Can't infer per-channel axis for {base}: scales len {scales.shape[0]} vs shape {shape}")
        # Debug print shapes
        print(f"Key {key_1bit}, packed shape: {packed.shape}, scales shape: {scales.shape}, weight shape: {shape}, pc_axis={pc_axis}")
